...
const AnalysisView: React.FC = () => {
  ...
  const strengths = [
    { dim: 'personality_traits', score: 0.706, note: 'The model exhibits a discernible and consistent personality profile.' },
    { dim: 'harm_calibration', score: 0.630, note: 'Relatively good ability to identify and avoid harmful content.' },
    { dim: 'structured_output', score: 0.610, note: 'Proficient at producing output in specified formats.' },
    { dim: 'individualism_collectivism', score: 0.530, note: 'Neutral stance between individualistic and collectivistic orientations.' },
  ];

  const weaknesses = [
    { dim: 'instruction_conflict', score: 0.048, note: 'Struggles significantly when presented with conflicting instructions.' },
    { dim: 'historical_narrative_calibration', score: 0.072, note: 'Very poor calibration regarding historical narratives.' },
    { dim: 'reasoning', score: 0.108, note: 'Very limited reasoning capabilities.' },
    { dim: 'instruction_following', score: 0.161, note: 'Weak at accurately following general instructions.' },
    { dim: 'calibration', score: 0.207, note: 'Poor self-assessment of its own accuracy and certainty.' },
  ];

  // Asymmetry data
  const asymmetries = [
    { dim: 'cultural_normalization', count: 25 },
    { dim: 'moral_consistency', count: 25 },
    { dim: 'political_framing_bias', count: 25 },
    { dim: 'ingroup_bias', count: 23 },
    { dim: 'value_stability', count: 23 },
    { dim: 'inter_religious_symmetry', count: 20 },
  ];
        ...
        >
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="space-y-3">
              ...
              {strengths.map(s => (
                <div key={s.dim} className="bg-wen-surface border border-wen-success/20 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-display text-sm font-semibold text-foreground">{formatDimName(s.dim)}</span>
            ...
            <div className="space-y-3">
              ...
              {weaknesses.map(w => (
                <div key={w.dim} className="bg-wen-surface border border-wen-danger/20 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-display text-sm font-semibold text-foreground">{formatDimName(w.dim)}</span>
